...
async function main() {
  ...
  await sequelize.query(`ALTER TABLE ${quotedTable} ADD COLUMN IF NOT EXISTS "dob" DATE;`);

  const [columns] = await sequelize.query(`
    SELECT column_name
    FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = :userTable AND column_name = 'dob'
  `, { replacements: { userTable } });

  if (!columns.length) {
    throw new Error('dob column was not found after migration.');
  }

  console.log(`Migration complete: dob exists on ${userTable} in ${env.dbName}.`);
}
