...
async function main() {
  ...
  await sequelize.query(`ALTER TABLE ${quotedTable} ADD COLUMN IF NOT EXISTS "avatar_url" VARCHAR(255);`);

  const [columns] = await sequelize.query(`
    SELECT column_name
    FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = :userTable AND column_name = 'avatar_url'
  `, { replacements: { userTable } });

  if (!columns.length) {
    throw new Error('avatar_url column was not found after migration.');
  }

  console.log(`Migration complete: avatar_url exists on ${userTable} in ${env.dbName}.`);
}
